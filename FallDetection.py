...
def DrawPoints(count=0):
    ...
    if None not in points:
        ...
        cv2.putText(frame, "{}".format(headHeight), points[0], cv2.FONT_HERSHEY_COMPLEX, .6, (255, 50, 0), 2,
                    lineType=cv2.LINE_AA)
        cv2.putText(frame, "{}".format(torsoHeight), points[8], cv2.FONT_HERSHEY_COMPLEX, .6, (255, 50, 0), 2,
                    lineType=cv2.LINE_AA)
...
if __name__ == '__main__':
    while cv2.waitKey(1) < 0:
        ...
        # error check
        print("Head Height: {}\nHead Heigh previous: {} \nTorso Height: {}".format(headHeight, headHeight_prev,
                                                                                   torsoHeight))
